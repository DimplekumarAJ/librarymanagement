...
import { registerAccount, loginAccount, getAccounts, getBooks, addBook, updateBook, deleteBook, getIssues, issueBook, returnBook, renewBook, getReservations, addReservation, cancelReservation } from "./api";
...
/* ============================================================================
   DESIGN TOKENS
   Primary: Navy (#07152B) / Royal Blue (#2F66C9) / Champagne Gold (#D0A53B)
...
async function actionIssueBook({ studentId, bookId, dueDate, byAccount }) {
  // The actual transaction is performed by the backend/MySQL.
  // This keeps Issue Book consistent for all users and survives refresh.
  ...
  return {
    ...issue,
    id: issue.id,
    bookId: Number(issue.book_id ?? issue.bookId),
    studentId: issue.student_id ?? issue.studentId,
    issueDate: issue.issue_date ?? issue.issueDate,
    dueDate: issue.due_date ?? issue.dueDate,
    returnDate: issue.return_date ?? issue.returnDate ?? null,
    status: issue.status,
    renewCount: Number(issue.renew_count ?? issue.renewCount ?? 0),
    issuedBy: issue.issued_by
      ? { id: issue.issued_by, name: issue.issued_by_name }
      : issue.issuedBy,
    returnedBy: null,
  };
}
...
async function actionReturnBook({ issueId, byAccount, finePerDay }) {
  // MySQL is the source of truth for returns.
  ...
  const rawIssue = result?.issue || result;

  const issue = rawIssue
    ? {
        ...rawIssue,
        id: rawIssue.id,
        bookId: Number(rawIssue.book_id ?? rawIssue.bookId),
        studentId: rawIssue.student_id ?? rawIssue.studentId,
        issueDate: rawIssue.issue_date ?? rawIssue.issueDate,
        dueDate: rawIssue.due_date ?? rawIssue.dueDate,
        returnDate: rawIssue.return_date ?? rawIssue.returnDate ?? null,
        status: rawIssue.status,
        renewCount: Number(rawIssue.renew_count ?? rawIssue.renewCount ?? 0),
        issuedBy: rawIssue.issued_by
          ? { id: rawIssue.issued_by, name: rawIssue.issued_by_name }
          : rawIssue.issuedBy,
        returnedBy: rawIssue.returned_by
          ? { id: rawIssue.returned_by, name: rawIssue.returned_by_name }
          : rawIssue.returnedBy ?? null,
      }
    : null;
...
async function actionRenewBook({ issueId, studentId, days = 14 }) {
  const issues = await getIssues();
  const issue = (Array.isArray(issues) ? issues : []).find((i) => String(i.id) === String(issueId) && String(i.student_id ?? i.studentId) === String(studentId));
  ...
  const currentDue = String(issue.due_date ?? issue.dueDate ?? "").slice(0, 10);
  const newDue = addDays(currentDue, Number(days));
  const updated = await renewBook(issueId, newDue, studentId);
  return {
    ...updated,
    id: updated.id,
    bookId: Number(updated.book_id ?? updated.bookId),
    studentId: updated.student_id ?? updated.studentId,
    issueDate: updated.issue_date ?? updated.issueDate,
    dueDate: updated.due_date ?? updated.dueDate,
    returnDate: updated.return_date ?? updated.returnDate ?? null,
    status: updated.status,
    renewCount: Number(updated.renew_count ?? updated.renewCount ?? 0),
  };
}

async function actionReserveBook({ studentId, bookId }) {
  const [reservationRows, bookRows] = await Promise.all([getReservations(), getBooks()]);
  const reservations = Array.isArray(reservationRows) ? reservationRows : [];
  const books = Array.isArray(bookRows) ? bookRows : [];
  const normalizedBookId = Number(bookId);
  const already = reservations.some((r) =>
    Number(r.book_id ?? r.bookId) === normalizedBookId &&
    String(r.student_id ?? r.studentId) === String(studentId) &&
    String(r.status) !== "Cancelled"
  );
  if (already) throw new Error("You already have an active reservation for this book.");

  const activeForBook = reservations.filter((r) =>
    Number(r.book_id ?? r.bookId) === normalizedBookId && String(r.status) !== "Cancelled"
  );
  const book = books.find((b) => Number(b.id) === normalizedBookId);
  const queuePosition = activeForBook.length + 1;
  const availableCopies = Number(book?.available_copies ?? book?.availableCopies ?? 0);
  const status = queuePosition === 1 && availableCopies > 0 ? "Ready for Pickup" : "Waiting";
  ...
  return {
    ...created,
    id: created.id,
    bookId: Number(created.book_id ?? created.bookId),
    studentId: created.student_id ?? created.studentId,
    reservedOn: String(created.reserved_on ?? created.reservedOn ?? todayStr()).slice(0, 10),
    queuePosition: Number(created.queue_position ?? created.queuePosition ?? queuePosition),
...
async function actionMarkNotificationsRead(userId) {
  const notifications = await loadKey(KEYS.notifications, []);
  const next = notifications.map((n) => (n.userId === userId ? { ...n, read: true } : n));
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [books, accounts, issues, reservations, fines, notifications, activity, settings] = await Promise.all([
        ...
        getAccounts().then((rows) => (Array.isArray(rows) ? rows : []).map((a) => ({
          ...a,
          id: a.id,
          role: a.role,
          name: a.name,
          usn: a.usn ?? null,
          staffId: a.staff_id ?? a.staffId ?? null,
          email: a.email,
          phone: a.phone ?? null,
          department: a.department ?? null,
          password: a.password,
          cardId: a.card_id ?? a.cardId ?? null,
          joinedDate: a.joined_date ?? a.joinedDate ?? null,
        }))),
        getIssues().then((rows) => (Array.isArray(rows) ? rows : []).map((i) => ({
          ...i,
          id: i.id,
          bookId: Number(i.book_id ?? i.bookId),
          studentId: i.student_id ?? i.studentId,
          issueDate: i.issue_date ?? i.issueDate,
          dueDate: i.due_date ?? i.dueDate,
          returnDate: i.return_date ?? i.returnDate ?? null,
          status: i.status,
          renewCount: Number(i.renew_count ?? i.renewCount ?? 0),
          issuedBy: i.issued_by
            ? { id: i.issued_by, name: i.issued_by_name }
            : i.issuedBy,
          returnedBy: i.returned_by
            ? { id: i.returned_by, name: i.returned_by_name }
            : i.returnedBy ?? null,
        }))),
        getReservations().then((rows) => (Array.isArray(rows) ? rows : []).map((r) => ({
          ...
          reservedOn: String(r.reserved_on ?? r.reservedOn ?? "").slice(0, 10),
          queuePosition: Number(r.queue_position ?? r.queuePosition ?? 0),
          status: r.status,
        }))), loadKey(KEYS.fines, []), loadKey(KEYS.notifications, []),
...
const COLLEGE_LOGO = "/college-logo.jpeg";
const COLLEGE_HEADER = "/college-header.jpeg";

function CollegePageBanner({ compact = false }) {
  return (
    <div className={`relative overflow-hidden bg-white border border-indigo-100 shadow-[0_12px_35px_rgba(31,41,55,0.08)] ${compact ? "rounded-2xl" : "rounded-3xl"}`}>
      <div className="absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r from-[#EF4444] via-[#7C3AED] to-[#1D4ED8]" />
      <div className={`relative flex items-center gap-4 ${compact ? "p-3" : "p-4 sm:p-5"}`}>
        <div className="hidden sm:flex w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-50 to-violet-50 border border-indigo-100 items-center justify-center overflow-hidden shrink-0">
          <img src={COLLEGE_LOGO} alt="GSSS SSFGC logo" className="w-11 h-11 object-contain" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-indigo-600">GSSS SSFGC</p>
          <h2 className="text-base sm:text-lg font-semibold text-slate-900 leading-tight">Geetha Shishu Shikshana Sangha(R) – GSSS SSFGC</h2>
          {!compact && <p className="mt-1 text-xs sm:text-sm text-slate-500">Library Management System · Smart campus library experience</p>}
        </div>
        <div className="hidden md:block w-44 lg:w-56 shrink-0 rounded-2xl bg-slate-50 border border-slate-100 p-1.5">
          <img src={COLLEGE_HEADER} alt="GSSS SSFGC header" className="w-full h-16 object-contain rounded-xl bg-white" />
        </div>
      </div>
    </div>
  );
    ...
    return (
      <div className="min-h-screen w-full flex items-center justify-center bg-[#FAFAFB] p-6 sm:p-10">
        <div className="w-full max-w-sm">
          <div className="mb-6"><CollegePageBanner compact /></div>
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 text-white">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-white overflow-hidden shadow-lg">
              <img src={COLLEGE_LOGO} alt="GSSS SSFGC" className="w-full h-full object-contain" />
            </div>
            <div>
              <p className="font-semibold text-lg tracking-tight">GSSS SSFGC</p>
              <p className="text-xs text-indigo-100">Geetha Shishu Shikshana Sangha(R)</p>
            </div>
          </div>
          <div>
            ...
            <div className="mt-7 rounded-3xl bg-white/95 p-2 shadow-2xl max-w-md">
              <img src={COLLEGE_HEADER} alt="GSSS SSFGC" className="w-full h-28 object-contain rounded-2xl bg-white" />
            </div>
          ...
          <p className="text-indigo-200 text-xs">© 2026 GSSS SSFGC Library</p>
        </div>
      </div>
...
function Shell({ role, page, setPage, onLogout, name, children, notifCount, onBell, lastSync }) {
  ...
  return (
    <div className="min-h-screen bg-[#F9F6F0] flex text-[#10203A]">
      {mobileOpen && <div className="fixed inset-0 bg-[#07152B]/55 backdrop-blur-sm z-30 lg:hidden" onClick={() => setMobileOpen(false)} />}

      <aside className={`fixed lg:sticky top-0 h-screen w-64 lg:w-48 bg-[#07152B] text-white z-40 flex flex-col transition-transform duration-200 shadow-[12px_0_30px_rgba(7,21,43,0.16)] ${mobileOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
        <div className="px-4 pt-5 pb-4 border-b border-white/10">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-[#D0A53B] flex items-center justify-center shrink-0 overflow-hidden">
              <img src={COLLEGE_LOGO} alt="GSSS SSFGC" className="w-7 h-7 object-contain" />
            </div>
            <div className="min-w-0">
              <p className="font-bold text-white text-sm leading-tight truncate">GSSS SSFGC</p>
              <p className="text-[8px] uppercase tracking-wide text-white/55 leading-tight truncate">Library Management</p>
            </div>
            <button onClick={() => setMobileOpen(false)} className="ml-auto lg:hidden text-white/60 hover:text-white"><X className="w-5 h-5" /></button>
          </div>
        </div>
          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[11px] font-medium transition-all ${
                  active ? "bg-[#2F66C9] text-white shadow-[0_5px_14px_rgba(47,102,201,0.28)]" : "text-white/78 hover:bg-white/8 hover:text-white"
                }`}
              >
                <item.icon className={`w-4 h-4 ${active ? "text-white" : "text-white/65"}`} />
        ...
        <div className="px-3 py-2 border-t border-white/10 flex items-center gap-2 text-[9px] text-white/45">
          <span className="w-1.5 h-1.5 rounded-full bg-[#D0A53B]" />
          {lastSync ? `Synced ${lastSync.toLocaleTimeString()}` : "Syncing…"}
        </div>
        <div className="p-3 border-t border-white/10">
          <button onClick={onLogout} className="w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-[11px] font-semibold text-white/70 hover:bg-white/10 hover:text-white transition">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 bg-[#F9F6F0]/95 backdrop-blur-xl border-b border-[#E7E0D2] flex items-center gap-3 px-4 sm:px-6 sticky top-0 z-20">
          <button onClick={() => setMobileOpen(true)} className="lg:hidden text-[#10203A]"><Menu className="w-5 h-5" /></button>
          <div className="relative hidden sm:block flex-1 max-w-xl">
            ...
            <input placeholder="Search books, students, transactions…" className="w-full pl-9 pr-4 py-2 rounded-xl bg-white border border-[#E7E0D2] text-xs placeholder:text-[#9B9FA6] focus:outline-none focus:border-[#2F66C9] focus:ring-4 focus:ring-[#2F66C9]/10 transition" />
          </div>
          <div className="ml-auto flex items-center gap-3">
            <div className="hidden md:flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-white border border-[#E7E0D2]">
              <span className="w-1.5 h-1.5 rounded-full bg-[#2F66C9]" />
              <span className="text-[10px] font-medium text-[#5E6570]">GSSS SSFGC Live</span>
            </div>
            <button onClick={onBell} className="relative w-9 h-9 rounded-xl hover:bg-white flex items-center justify-center transition">
              <Bell className="w-4.5 h-4.5 text-[#5E6570]" />
              {notifCount > 0 && <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-[#C79B35] ring-2 ring-[#F9F6F0]" />}
            </button>
            <div className="flex items-center gap-2 pl-2 border-l border-[#DED6C7]">
              <Avatar name={name} size={8} />
              <div className="hidden sm:block leading-tight">
                <p className="text-xs font-semibold text-[#10203A]">{name}</p>
                <p className="text-[9px] text-[#8A8F98]">{ROLE_LABEL[role]}</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-7 max-w-[1400px] w-full mx-auto">
          {!(role === "student" && page === "dashboard") && <CollegePageBanner />}
          <div className={role === "student" && page === "dashboard" ? "" : "mt-5"}>{children}</div>
        </main>
      </div>
    </div>
  );
...
function StudentDashboard({ setPage, openBook }) {
  ...
  const globalIssued = issues.filter((i) => i.status !== "Returned").length;
  const activeReservations = reservations.filter((r) => r.status !== "Cancelled").length;
  const outstandingFines = fines.filter((f) => f.status === "Unpaid").reduce((sum, f) => sum + Number(f.amount || 0), 0);

  const recentBooks = useMemo(() => [...books].sort((a, b) => Number(b.id) - Number(a.id)).slice(0, 4), [books]);
  const recentActivity = useMemo(() => {
    const events = [];
    issues.filter((i) => i.studentId === session.id).forEach((i) => {
      events.push({ icon: BookOpen, text: `Issued "${bookById(books, i.bookId)?.title || "a book"}"`, time: i.issueDate });
      if (i.returnDate) events.push({ icon: CheckCircle2, text: `Returned "${bookById(books, i.bookId)?.title || "a book"}"`, time: i.returnDate });
    });
    reservations.filter((r) => r.studentId === session.id).forEach((r) => events.push({ icon: Bookmark, text: `Reserved "${bookById(books, r.bookId)?.title || "a book"}"`, time: r.reservedOn }));
    return events.sort((a, b) => (a.time < b.time ? 1 : -1)).slice(0, 3);
  }, [issues, reservations, books, session.id]);

  const stats = [
    [BookOpen, "BOOKS AVAILABLE", totalCopies.toLocaleString()],
    [BookOpen, "CURRENTLY ISSUED", globalIssued],
    [Bookmark, "RESERVATIONS", activeReservations],
    [Wallet, "OUTSTANDING FINES", `₹${outstandingFines.toLocaleString()}`],
  ];

  return (
    <div className="space-y-5">
      <div className="pt-1">
        <p className="text-[9px] font-semibold uppercase tracking-[0.2em] text-[#B18425]">GSSS SSFGC • DIGITAL LIBRARY</p>
        <h1 className="text-2xl sm:text-[25px] font-semibold tracking-[-0.03em] text-[#10203A] mt-1">Good evening, {session.name.split(" ")[0]}</h1>
        <p className="text-xs text-[#68707C] mt-1">A calm, premium space to discover, borrow and manage knowledge.</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {stats.map(([Icon, label, value]) => (
          <div key={label} className="bg-white rounded-2xl border border-[#EEE8DC] px-4 py-4 shadow-[0_5px_18px_rgba(16,32,58,0.035)]">
            <p className="text-[8px] font-semibold uppercase tracking-wide text-[#68707C]">{label}</p>
            <p className="text-xl font-semibold text-[#10203A] mt-2">{value}</p>
          </div>
        ))}
      </div>

      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[#EEE8DC] p-5 shadow-[0_5px_18px_rgba(16,32,58,0.035)]">
          <h2 className="text-base font-semibold text-[#10203A]">Discover your next great read</h2>
          <p className="text-[10px] text-[#727984] mt-1">Search across the GSSS SSFGC collection by title, author, ISBN or category.</p>
          <div className="mt-4 flex items-center gap-2 p-1.5 rounded-xl bg-[#F8F4EC]">
            <input onKeyDown={(e) => e.key === "Enter" && setPage("search")} placeholder="Search books, authors, ISBN..." className="flex-1 min-w-0 bg-transparent px-2.5 py-2 text-[10px] text-[#10203A] placeholder:text-[#9B9FA6] outline-none" />
            <button onClick={() => setPage("search")} className="px-4 py-2 rounded-lg bg-[#07152B] text-white text-[9px] font-semibold hover:bg-[#10294B] transition">Search</button>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mt-3">
            {["Computer Science", "Electronics", "Literature", "Business"].map((cat, idx) => (
              <button key={cat} onClick={() => setPage("search")} className={`px-3 py-2.5 rounded-lg text-left text-[9px] border ${idx === 1 ? "bg-[#F6EED8] border-[#E9D9A9] text-[#6F5720]" : "bg-[#FAF8F4] border-[#EEE8DC] text-[#3E4652]"}`}>{cat}</button>
            ))}
          </div>
        </div>

        <div className="rounded-2xl bg-[#102443] text-white p-5 shadow-[0_8px_22px_rgba(16,36,67,0.14)]">
          <h2 className="text-sm font-semibold">Your library pulse</h2>
          <div className="mt-5 space-y-4 text-[10px] text-white/85">
            <p>{myIssues.length || 0} {myIssues.length === 1 ? "book" : "books"} currently issued</p>
            <p>{myReservations.length || 0} reservation{myReservations.length === 1 ? "" : "s"} active</p>
            <p className="text-[#E5C66D]">Fine balance: ₹{myFinesUnpaid.reduce((s, f) => s + Number(f.amount || 0), 0)}</p>
          </div>
          <button onClick={() => setPage("mybooks")} className="mt-6 text-[9px] font-semibold text-[#E5C66D] hover:text-white transition">Open my library →</button>
        </div>
      </div>

      <div className="bg-white rounded-2xl border border-[#EEE8DC] p-5 shadow-[0_5px_18px_rgba(16,32,58,0.035)]">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-base font-semibold text-[#10203A]">Recently added to the collection</h2>
          <button onClick={() => setPage("search")} className="text-[9px] font-semibold text-[#2F66C9]">Browse all →</button>
        </div>
        <div className="space-y-2.5">
          {recentBooks.length === 0 ? <p className="text-xs text-[#8A8F98]">No books available yet.</p> : recentBooks.map((book, idx) => (
            <button key={book.id} onClick={() => openBook(book.id)} className="w-full flex items-center gap-3 text-left group rounded-xl px-1 py-1 hover:bg-[#FAF8F4] transition">
              <div className={`w-7 h-7 rounded-lg shrink-0 ${idx === 2 ? "bg-[#D0A53B]" : "bg-[#2F66C9]"}`} />
              <div className="min-w-0 flex-1">
                <p className="text-[10px] font-semibold text-[#10203A] truncate group-hover:text-[#2F66C9]">{book.title}</p>
                <p className="text-[8px] text-[#7B828C] truncate">{book.author}</p>
              </div>
              <span className={`text-[8px] font-semibold ${Number(book.availableCopies) > 0 ? "text-[#2F66C9]" : "text-[#B18425]"}`}>{Number(book.availableCopies) > 0 ? "Available" : "Popular"}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[#EEE8DC] p-5 shadow-[0_5px_18px_rgba(16,32,58,0.035)]">
          <div className="flex items-center justify-between mb-3"><h2 className="text-sm font-semibold text-[#10203A]">Currently issued</h2><button onClick={() => setPage("mybooks")} className="text-[9px] font-semibold text-[#2F66C9]">View all →</button></div>
          {myIssues.length === 0 ? <p className="text-xs text-[#8A8F98] py-3">Nothing issued right now. Search the catalogue to borrow a book.</p> : <div className="space-y-2">{myIssues.slice(0, 3).map((item) => { const book = bookById(books, item.bookId); const remaining = daysBetween(today, item.dueDate); const status = computedIssueStatus(item, today); return <div key={item.id} className="flex items-center gap-3 p-2 rounded-xl bg-[#FAF8F4]"><div className="w-7 h-9 shrink-0"><BookCover book={book} size="sm" /></div><div className="min-w-0 flex-1"><p className="text-[10px] font-medium text-[#10203A] truncate">{book?.title}</p><p className="text-[8px] text-[#7B828C]">Due {item.dueDate} · {remaining >= 0 ? `${remaining} days left` : `${-remaining} days overdue`}</p></div><StatusBadge status={status} /></div>; })}</div>}
        </div>
        <div className="bg-white rounded-2xl border border-[#EEE8DC] p-5 shadow-[0_5px_18px_rgba(16,32,58,0.035)]"><h2 className="text-sm font-semibold text-[#10203A]">Recent activity</h2>{recentActivity.length === 0 ? <p className="text-xs text-[#8A8F98] mt-4">No activity yet.</p> : <div className="space-y-3 mt-4">{recentActivity.map((a, i) => <div key={i} className="flex gap-2.5"><div className="w-7 h-7 rounded-lg bg-[#F6EED8] text-[#B18425] flex items-center justify-center shrink-0"><a.icon className="w-3.5 h-3.5" /></div><div><p className="text-[9px] text-[#3E4652]">{a.text}</p><p className="text-[8px] text-[#9B9FA6] mt-0.5">{a.time}</p></div></div>)}</div>}</div>
      </div>

      <div className="pt-1 text-[8px] uppercase tracking-[0.18em] text-[#B18425]">Premium digital library experience</div>
    </div>
  );
}
...
function MyBooks() {
  ...
  // MySQL is the source of truth for issued books.
  // Accept both the normalized frontend fields and raw MySQL snake_case fields
  // so the page keeps working even if the API response shape changes slightly.
  const currentStudentId = String(session?.id ?? "").trim();

  const mine = issues.filter((i) => {
    const issueStudentId = String(i.studentId ?? i.student_id ?? "").trim();
    const issueStatus = String(i.status ?? "").trim().toLowerCase();

    return (
      issueStudentId === currentStudentId &&
...
function useCirculationStats() {
  ...
  const issuedToday = issues.filter(
    (i) => String(i.issueDate ?? "").slice(0, 10) === today
  ).length;
  const returnedToday = issues.filter(
    (i) => String(i.returnDate ?? "").slice(0, 10) === today
  ).length;
...
function AppShell() {
  ...
  const transactionRows = useMemo(() => {
    ...
    lib.issues.forEach((i) => {
      rows.push([
        i.id,
        accountById(lib.accounts, i.studentId)?.name || "Unknown student",
        bookById(lib.books, i.bookId)?.title || "Unknown book",
        "Issue",
        i.issueDate,
      ]);
      if (i.returnDate) {
  ...
  let content = null;
  const currentRole =
    String(session.role || "").toLowerCase() === "administrator"
      ? "admin"
  ...
  if (currentRole === "student") {
    ...
    content = {
      ...
      transactions: <GenericTable title="Transactions" subtitle="Full issue and return log." columns={["ID", "Student", "Book", "Type", "Date"]} rows={transactionRows} />,
      reservations: <LibrarianReservationsTable />,
  ...
  return (
    <Shell role={currentRole} page={page} setPage={setPage} onLogout={logout} name={session.name} notifCount={notifCount} onBell={() => setNotifOpen(true)} lastSync={lib.lastSync}>
      {content}
