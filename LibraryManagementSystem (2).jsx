...
async function actionRegister({ role, name, idOrUsn, email, department, password }) {
  const accounts = await loadKey(KEYS.accounts, []);
  const idLower = idOrUsn.trim().toLowerCase();
  const emailLower = email.trim().toLowerCase();
  const clash = accounts.find((a) =>
    (a.usn && a.usn.toLowerCase() === idLower) ||
    a.id.toLowerCase() === idLower ||
    a.email.toLowerCase() === emailLower
  );
  if (clash) throw new Error("An account with that ID or email already exists.");
  const account = {
    id: uid(role),
    role,
    name: name.trim(),
    ...
    department,
    joinedDate: todayStr(),
    ...(role === "student" ? { usn: idOrUsn.trim(), cardId: `LIB-${department}-${Math.floor(1000 + Math.random() * 9000)}` } : { staffId: idOrUsn.trim() }),
  };
  accounts.push(account);
  await saveKey(KEYS.accounts, accounts);
  return account;
}
...
async function actionLogin({ role, identifier, password }) {
  ...
  const found = accounts.find((a) =>
    ...
    (a.id.toLowerCase() === idLower ||
      (a.usn && a.usn.toLowerCase() === idLower) ||
      (a.staffId && a.staffId.toLowerCase() === idLower) ||
...
async function actionAddBook(bookData) {
  const books = await loadKey(KEYS.books, []);
  const nextId = books.reduce((m, b) => Math.max(m, b.id), 0) + 1;
  const book = { id: nextId, rating: 0, reviews: 0, ...bookData, availableCopies: bookData.totalCopies };
  books.push(book);
  await saveKey(KEYS.books, books);
  return book;
}
...
async function actionUpdateBook(bookId, patch) {
  ...
  const totalCopies = patch.totalCopies !== undefined ? Number(patch.totalCopies) : prev.totalCopies;
  const delta = totalCopies - prev.totalCopies;
  books[idx] = { ...prev, ...patch, totalCopies, availableCopies: Math.max(0, prev.availableCopies + delta) };
  await saveKey(KEYS.books, books);
  return books[idx];
}
...
async function actionDeleteBook(bookId) {
  const books = await loadKey(KEYS.books, []);
  await saveKey(KEYS.books, books.filter((b) => b.id !== bookId));
}
...
async function actionReserveBook({ studentId, bookId }) {
  const [reservations, books] = await Promise.all([loadKey(KEYS.reservations, []), loadKey(KEYS.books, [])]);
  const already = reservations.some((r) => r.bookId === bookId && r.studentId === studentId && r.status !== "Cancelled");
  if (already) throw new Error("You already have an active reservation for this book.");
  const activeForBook = reservations.filter((r) => r.bookId === bookId && r.status !== "Cancelled");
  const book = books.find((b) => b.id === bookId);
  const queuePosition = activeForBook.length + 1;
  const status = queuePosition === 1 && book && book.availableCopies > 0 ? "Ready for Pickup" : "Waiting";
  const reservation = { id: uid("RES"), bookId, studentId, reservedOn: todayStr(), queuePosition, status };
  reservations.push(reservation);
  await saveKey(KEYS.reservations, reservations);
  return reservation;
}

async function actionCancelReservation({ reservationId }) {
  const reservations = await loadKey(KEYS.reservations, []);
  const target = reservations.find((r) => r.id === reservationId);
  if (!target) return;
  const remaining = reservations
    .filter((r) => r.id !== reservationId)
    .map((r) => (r.bookId === target.bookId && r.queuePosition > target.queuePosition)
      ? { ...r, queuePosition: r.queuePosition - 1 }
      : r);
  await saveKey(KEYS.reservations, remaining);
}
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const [books, accounts, issues, reservations, fines, notifications, activity, settings] = await Promise.all([
        loadKey(KEYS.books, []), loadKey(KEYS.accounts, []), loadKey(KEYS.issues, []),
        loadKey(KEYS.reservations, []), loadKey(KEYS.fines, []), loadKey(KEYS.notifications, []),
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (!fresh) { setSessionRaw(null); return; }
    if (JSON.stringify(fresh) !== JSON.stringify(session)) setSessionRaw(fresh);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [db.accounts, db.loaded]);
